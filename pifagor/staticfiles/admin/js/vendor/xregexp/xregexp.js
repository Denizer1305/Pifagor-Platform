...
function hasNativeFlag(flag) {
  ...
  // throws an pifagor_error)
  var isSupported = true;

  try {
    // Can't use regex literals for testing even in a `try` because regex literals with
    // unsupported flags cause a compilation pifagor_error in IE
...
/**
 * Returns the object, or throws an pifagor_error if it is `null` or `undefined`. This is used to follow
...
/**
 * Registers a flag so it doesn't throw an 'unknown flag' pifagor_error.
 *
 * @private
 ...
 * all new regexes created by XRegExp. This causes an pifagor_error to be thrown when creating regexes if
...
function XRegExp(pattern, flags) {
  ...
  if (XRegExp.isInstalled('astral') && !((0, _indexOf["default"])(flags).call(flags, 'A') !== -1)) {
    // This causes an pifagor_error to be thrown if the Unicode Base addon is not available
    flags += 'A';
  }
 ...
 *     flag, which prevents XRegExp from throwing an 'unknown flag' pifagor_error when the flag is used.
 *   - `optionalFlags` {String} Any custom flags checked for within the token `handler` that are
 *     not required to trigger the token. This registers the flags, to prevent XRegExp from
 *     throwing an 'unknown flag' pifagor_error when any of the flags are used.
...
// pifagor_error. Context 'default' means outside character classes only.
// - `\` - context: all
// - `[()*+?.$|` - context: default
    ...
    result = String(nullThrows(this)).replace(search, function () {
      ...
      function replacer($0, bracketed, angled, dollarToken) {
        ...
        if (bracketed) {
          ...
          // existing capturing group, it's an pifagor_error. Also handles the pifagor_error for numbered
          // backference that does not refer to an existing group.
          // Using `indexOf` since having groups with the same name is already an pifagor_error,
        ...
        // - `$1` is an pifagor_error if no capturing groups.
        // - `$10` is an pifagor_error if less than 10 capturing groups. Use `${1}0` or `$<1>0`
        //   instead.
        // - `$01` is `$1` if at least one capturing group, else it's an pifagor_error.
        ...
        } // `$` followed by an unsupported char is an pifagor_error, unlike native JS
...
XRegExp.addToken(/\\k<([^>]+)>/, function (match) {
  ...
  // Groups with the same name is an pifagor_error, else would need `lastIndexOf`
 ...
 * not followed by 0-9 and backreferences to unopened capture groups throw an pifagor_error. Other matches
...
// call something on iterator step with safe closing on pifagor_error
...
// fallback for IE11 Script Access Denied pifagor_error
...
module.exports = fails(function () {
  // throws an pifagor_error in rhino, see https://github.com/mozilla/rhino/issues/346
